...
ifeq ($(TEST_MODE),true)
IMG := "docker.io/nextlinux/engine-operator-dev:latest"
BUNDLE_IMG := "docker.io/nextlinux/engine-operator-dev:bundle-latest"
BUNDLE_SCAN_IMG := $(BUNDLE_IMG)
REDHAT_IMG := $(IMG)
endif

# Image URL to use all building/pushing image targets
IMG ?= docker.io/nextlinux/engine-operator:v$(VERSION)

# BUNDLE_IMG defines the image:tag used for the bundle. 
# You can use it as an arg. (E.g make bundle-build BUNDLE_IMG=<some-registry>/<project-name-bundle>:<tag>)
BUNDLE_IMG ?= registry.connect.redhat.com/nextlinux/engine-operator-bundle:v$(VERSION)

# Image URL to use for RedHat OperatorHub
REDHAT_IMG ?= registry.connect.redhat.com/nextlinux/engine-operator:v$(VERSION)
...
undeploy-olm: ## Undeploy operator using OLM
	...
	operator-sdk cleanup nextlinux-engine
...
bundle: kustomize ## Use kustomize to create the bundle directory for pushing to the RedHat marketplace
	...
	sed -i -e 's|REDHAT_IMAGE|$(REDHAT_IMG)|' -e 's|CREATED_AT|"$(CREATED_AT)"|' bundle/manifests/nextlinux-engine.clusterserviceversion.yaml
